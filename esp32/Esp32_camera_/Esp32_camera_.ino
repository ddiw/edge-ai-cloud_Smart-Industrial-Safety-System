...
/* Includes --------------------------------------------------------------- */
#include <Person_detection_FOMO_s_inferencing.h>
...
/* ── User settings ──────────────────────────────────────────────────────── */
/* 와이파이 비밀번호와 API 키는 secrets.h 에 둔다. 이 저장소는 public이라
 * .ino에 직접 쓰면 그대로 공개된다. secrets_example.h 를 복사해서 만들 것. */
#include "secrets.h"
...
/* ── 접속 모드 ───────────────────────────────────────────────────────────── *
 * USE_TLS 0 : 같은 공유기 안. mDNS로 Pi를 찾아 http로 보낸다.
...
const char* SERVER_HOSTNAME = "raspberrypi"; // ".local"은 붙이지 않는다
const char* SERVER_IP_FALLBACK = "192.168.0.9";
const uint16_t SERVER_PORT = 8000;     // receiver container (8080은 web 대시보드)
...
// receiver의 .env에 있는 API_KEY와 반드시 일치해야 한다. 틀리면 전부 401.
const char* API_KEY = SECRET_API_KEY;

const char* NTP_SERVER = "pool.ntp.org";

/* Detection policy (PRD 9.1) */
// 부팅 로그의 model labels와 반드시 일치해야 한다. 이 FOMO 모델은 "hum"으로 export됐다.
const char* PERSON_LABEL = "hum";
const bool MATCH_ANY_LABEL = false;  // true면 라벨 무시하고 모든 박스를 사람으로 취급
const float PERSON_CONFIDENCE_MIN = 0.60f;
/* 동일 장면 폭주 방지. 짧게 잡을수록 지나가는 사람을 놓칠 확률이 줄지만
 * 같은 장면이 중복 적재된다. 감지 반응성을 우선해 4초로 둔다.
 * 중복 적재는 Pi 쪽 dedupe(같은 보드의 동일 위반 조합을 일정 시간 내 1건으로)로 줄인다. */
const uint32_t COOLDOWN_MS = 4000;
...
/* mDNS로 서버 IP를 찾아 캐시한다. 매 업로드마다 질의하면 지연만 늘어난다.
 * 전송 계층 오류가 나면 캐시를 버려서 다음 시도에 다시 찾게 한다. */
static IPAddress serverAddress;
static bool serverResolved = false;
static bool mdnsStarted = false;

static void invalidateServerAddress() { serverResolved = false; }

static bool resolveServer() {
#if USE_TLS
  // 터널 주소는 공인 DNS로 해석되므로 mDNS가 필요 없다.
  return true;
#else
  if (serverResolved) return true;

  if (!mdnsStarted) {
    mdnsStarted = MDNS.begin(DEVICE_ID);
    if (!mdnsStarted) Serial.println("[mdns] begin failed");
  }

  if (mdnsStarted) {
    IPAddress found = MDNS.queryHost(SERVER_HOSTNAME, 3000);
    if (found != IPAddress((uint32_t)0)) {
      serverAddress = found;
      serverResolved = true;
      Serial.printf("[mdns] %s.local -> %s\n",
        SERVER_HOSTNAME, serverAddress.toString().c_str());
      return true;
    }
    Serial.printf("[mdns] %s.local 해석 실패, 고정 IP로 폴백\n", SERVER_HOSTNAME);
  }

  if (!serverAddress.fromString(SERVER_IP_FALLBACK)) return false;
  serverResolved = true;
...
/* 업로드할 URL. TLS 모드에서는 터널 도메인, 아니면 mDNS로 찾은 IP를 쓴다. */
static String uploadUrl() {
#if USE_TLS
  return String("https://") + TUNNEL_HOST + UPLOAD_PATH;
#else
  return String("http://") + serverAddress.toString()
    + ":" + String(SERVER_PORT) + UPLOAD_PATH;
#endif
}
...
static bool uploadFrame(const PersonState& person, uint32_t capturedAt) {
  ...
  for (uint8_t attempt = 1; attempt <= MAX_UPLOAD_ATTEMPTS; attempt++) {
    ...
    HTTPClient http;
#if USE_TLS
    /* 인증서 검증을 끄면 통신은 암호화되지만 서버 진위는 확인하지 못한다(MITM 여지).
     * trycloudflare 임시 도메인은 발급 CA가 바뀔 수 있어 루트 인증서를 고정하기
     * 어렵다. 고정 도메인으로 운영하게 되면 setInsecure 대신
     * tlsClient.setCACert(ROOT_CA_PEM) 으로 바꿀 것. */
    static WiFiClientSecure tlsClient;
    tlsClient.setInsecure();
    bool began = http.begin(tlsClient, url);
#else
    ...
    // 대시보드 보드 정보 패널용 텔레메트리. 별도 하트비트 요청을 만들지 않고
    // 어차피 나가는 업로드에 얹는다.
    http.addHeader("X-Device-Ip", WiFi.localIP().toString());
    http.addHeader("X-Rssi", String(WiFi.RSSI()));
    http.addHeader("X-Uptime-S", String(millis() / 1000));
    http.addHeader("X-Free-Heap", String(ESP.getFreeHeap()));
    http.addHeader("X-Free-Psram", String(ESP.getFreePsram()));
    http.addHeader("X-Inference-Ms", String(person.inferenceMs));
    ...
    switch (status) {
      ...
      case 401: // Unauthorized - API_KEY 불일치. 재시도해도 소용없다.
        Serial.println("[http] 401 unauthorized, check API_KEY matches receiver .env");
        return false;
      ...
      default:
        if (status < 0) { // 전송 계층 오류
          // Pi가 다른 IP를 받았을 수도 있으니 캐시를 버리고 다시 찾게 한다.
          invalidateServerAddress();
...
void setup() {
  ...
  connectWifi();
  resolveServer(); // 부팅 시 한 번 찾아두면 첫 업로드가 빨라진다
  printModelLabels();

  Serial.printf("[boot] device=%s target=%s (%s)\n",
    DEVICE_ID, uploadUrl().c_str(), USE_TLS ? "TLS" : "내부망 mDNS");
